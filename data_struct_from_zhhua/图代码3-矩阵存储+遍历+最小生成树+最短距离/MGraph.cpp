...
{ // 算法7.15
  ...
  for (v=0; v<G.vexnum; ++v) {
    ...
    if (D[v] < INFINITY) { P[v][v0] = TRUE;  P[v][v] = TRUE; }//v0到v有路可通。
  ...
  for (i=1; i<G.vexnum; ++i) {         // 其余G.vexnum-1个顶点
    ...
    for (w=0; w<G.vexnum; ++w)
      if (!final[w])                           // w顶点在V-S中
        if (D[w]<min) { v = w;  min = D[w]; }  // w顶点离v0顶点更近，最后求得v是离v0最近的顶点
    final[v] = TRUE;                       // 离v0顶点最近的v加入S集
    for (w=0; w<G.vexnum; ++w)             // 更新当前最短路径及距离
      //if (!final[w] && (min+G.arcs[v][w].adj)<D[w]) { //表明v0-经过v到w的路径长度比当前v0到w的路径长度要短
	  if (!final[w] && ((double)min+G.arcs[v][w].adj)<D[w]) { //表明v0-经过v到w的路径长度比当前v0到w的路径长度要短
      ...
      //if (!final[w] && G.arcs[v][w].adj!=INFINITY&&(min+G.arcs[v][w].adj<D[w])) {
        ...
        for(j=0;j<G.vexnum;j++) P[w][j] = P[v][j]; //修正P[w],第v行赋值于第w行
...
void ShortestPath_FLOYD(MGraph G, PathMatrix P[], DistancMatrix &D) { //p是一个一维数组，同时每个元素又是一个二维数组，即三维数组
  // 算法7.16
  ...
  for (v=0; v<G.vexnum; ++v)// 各对结点之间初始已知路径及距离
    for (w=0; w<G.vexnum; ++w) {
      ...
      if (D[v][w] < INFINITY) {//D[v][w] < INFINITY  表示 v到w有路可走
  ...
  for (u=0; u<G.vexnum; ++u)
    for (v=0; v<G.vexnum; ++v)
      for (w=0; w<G.vexnum; ++w)
        if ((double)D[v][u]+D[u][w] < D[v][w]) {  // 从v经u到w的一条路径更短,前面取double防止两个整数相加出现溢出
